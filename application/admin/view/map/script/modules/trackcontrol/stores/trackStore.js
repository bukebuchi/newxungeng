...
var TrackStore = Reflux.createStore({
        ...
        Urls.jsonp(Urls.columnsList, params, function(data) {
            ...
            if (data.status === 0) {
                data.columns.map(function(item) {
                    ...
                    if(item.column_key==='巡逻一大队')
                    {
                    this.trigger('servicetype', 0);	
                    }
                    else if(item.column_key==='巡逻二大队' )
                    {
                    	this.trigger('servicetype', 1);	
                    }
                    else if(item.column_key==='街道巡逻队' )
                    {
                    	this.trigger('servicetype', 2);	
                    }
                    else
                    {
    ...
    onBoundsearchentity(display) {
    	console.log('onBoundsearchentity');
        let that = this;
        let time = new Date();
                ...
                Urls.jsonp(Urls.boundsearchEntity, params, function (data) {
                    if (data.status === 0) {
                        data.entities.map(function (eitem, eindex) {
                            ...
                            let entity_status = 0;
                              let grouptype = 0;
                             if(eitem.group ==='巡逻一大队'){
                                	grouptype = 0;

                                }
                                else if(eitem.group ==='巡逻二大队')
                                {
                                	grouptype = 1;
                                }
                                else if(eitem.group ==='街道巡逻队')
                                {
                                	grouptype = 2;
                                }
                            console.log(data);
        ...
        if (boundsDistance < boundsLimit) {
        	console.log('getBoundsearchResultOnce');
            getBoundsearchResultOnce(1);

        } else {
        	getBoundsearchResultTable();
        	console.log('getBoundsearchResultTable');
    ...
    setAllEntities: function(data) {
        ...
        data.entities.map(function (item, index) {
            ...
            if (Commonfun.getOnlineStatus(item.latest_location.loc_time) === 0) {
                ...
                console.log('setAllEntities');
                console.log(item);
                if(item.group==='巡逻一大队')
                {
                	descIndex = desc === '静止' ? 1 : 0;
                }
                else if(item.group==='巡逻二大队')
                {
                	descIndex = desc === '静止' ? 4 : 3;
                }
                else
                {
                	descIndex = desc === '静止' ? 7 : 6;
                }
